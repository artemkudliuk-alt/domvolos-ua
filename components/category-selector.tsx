    ...
    <div className="flex flex-col gap-2.5 sm:flex-row sm:items-center sm:justify-between py-1 border-b border-slate-100 pb-3.5">
      ...
      <div className="relative w-full sm:w-72">
        <select
          ...
          className="w-full cursor-pointer appearance-none rounded-2xl border border-slate-200 bg-white/95 px-4 py-2.5 pr-10 text-base font-medium text-slate-800 shadow-xs transition-all hover:border-ink/30 focus:border-ink focus:outline-none focus:ring-2 focus:ring-ink/10"
        >
          {categories.map((c) => (
      ...
      {/* Quick-Filter Chips (wrap instead of a hidden horizontal scroller) */}
        ...
        {categories.slice(0, 5).map((category) => {
          ...
          return (
            <button
              ...
              className={[
                "whitespace-nowrap rounded-full border px-3.5 py-1.5 text-sm font-light tracking-[0.01em] transition-all duration-200 active:scale-95",
                isActive
